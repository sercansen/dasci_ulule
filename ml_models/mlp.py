...
def dataframe_to_dataloaders(dataframe):
    ...
    dataframe.drop(columns=['amount_raised', 'comments_count', 'fans_count',
                   'goal_raised', 'id', 'percent', 'supporters_count'], axis=1, inplace=True)
...
def training(device, train_loader, valid_loader, model, criterion, optimizer, n_epochs=75):
    ...
    for epoch in range(n_epochs):
        ...
        if valid_loss <= valid_loss_min:
            torch.save(model.state_dict(), './ml_models/model.pt')
  ...
  # initialize lists to monitor test loss and accuracy
    ...
    for data, label in test_loader:
        ...
        correct = (np.squeeze(output.eq(label.data.view_as(output)))
                   ).cpu().numpy()  # compare predictions to true label
...
def mlp(dataframe) -> str:
    ...
    n_epochs = 75
    train_loader, valid_loader, test_loader, shape = dataframe_to_dataloaders(
        dataframe)
    ...
    train_losses, valid_losses = training(
        device, train_loader, valid_loader, model_MLP, criterion, optimizer, n_epochs)
    ...
    model_MLP.load_state_dict(torch.load(
        "./ml_models/model.pt", map_location=device))
